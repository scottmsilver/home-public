...
|---|---|---|---|---|---|---|
| **home** (gateway) | Python/Flask | WS (pentair), WS (fans), HTTP poll (gate) | SSE + REST | silver-oauth broker → JWT | yes (canonical) | home.example.com |
| **pentair** | Rust/Axum | Pentair TCP protocol | WebSocket + REST | Firebase (mobile) | yes | pool.example.com |
| **unifi-gate** | Python/Flask | UniFi Access/Protect HTTP | WS-ish + REST | Cloudflare Worker + Firebase + KV | yes | gate.example.com |
| **fans** | Python/aiohttp | Modern Forms HTTP + Leviton cloud | WebSocket + REST | silver-oauth broker → JWT | yes | patio.example.com |
...
- **Self-approve on-network (REQUIRED).** Today a user physically on the network can
  ...
  - **Trusted-network definition (DECIDED + hardened: positive local-origin check).**
    ...
    **IP-literal, `localhost`, or an explicitly allow-listed `local_hosts` name** — plus
    ...
    never a victim IP), so the running deployment (reached at `192.168.1.10:8099`) is
...
- 301 `gate./pool./patio.` → `home.example.com` tabs; remove their tunnels/Worker routes.
