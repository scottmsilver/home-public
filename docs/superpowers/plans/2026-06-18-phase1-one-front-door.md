...
- Tunnels (systemd `cloudflared-*.service`, configs in `~/.cloudflared/`):
  - `cloudflared-home` → tunnel `11111111-1111-1111-1111-111111111111` → `home.example.com` → `http://10.0.0.9:8099` (home container)
  - `cloudflared-gate` → tunnel `22222222-2222-2222-2222-222222222222` → `gate.example.com` → `http://10.0.0.240:8000` (unifi-gate container)
  - `cloudflared-pool` → tunnel `33333333-3333-3333-3333-333333333333` → `pool.example.com` → `http://localhost:8080` (pentair on host)
  - `config-patio.yml` → tunnel `44444444-4444-4444-4444-444444444444` → `patio.example.com` → `http://localhost:8095` (fans on host) — note: no running `cloudflared-patio.service` was found; confirm at execution.
...
- home container `home` IP `10.0.0.9`; inbound via Incus device `http8099` (`listen 0.0.0.0:8099` → `connect 127.0.0.1:8099`).
...
def test_grant_start_lan_issues_ticket_remote_forbidden():
    ...
    r = client.post("/api/auth/grant-start", headers={"Host": "192.168.1.10:8099"})
...
def test_grant_login_callback_approves_unlisted_email():
    ...
    g = client.post("/api/auth/grant-start", headers={"Host": "192.168.1.10:8099"}).get_json()
    ticket = g["ticket"]
...
def test_auth_me_reports_lan_flag():
    ...
    lan = client.get("/api/auth/me", headers={"Host": "192.168.1.10:8099"}).get_json()
...
$BR goto http://192.168.1.10:8099/ >/dev/null; sleep 5
...
Extract the emails and union them into `allowed_emails` in `home-instance/config/home.container.toml` (keep `you@gmail.com`). Example result:

```toml
allowed_emails = ["you@gmail.com", "person2@gmail.com", "person3@gmail.com"]
```
...
From a remote network (or phone off-wifi), open `https://home.example.com`, complete silver-oauth, confirm the app loads and controls work. Confirm a non-listed email is rejected (403 "not allowed").

- [ ] **Step 2: On-network self-approve works**

On the home wifi, open `http://192.168.1.10:8099`, tap the key ("Enable remote access"), complete the broker login, and confirm:
...
**Files:** Cloudflare dashboard (Rules → Redirect Rules) for zone `example.com`.

- [ ] **Step 1: Create three Single Redirect Rules**

For each of `gate.example.com`, `pool.example.com`, `patio.example.com`:
- When incoming `Hostname equals <host>` → Static redirect to `https://home.example.com/` , status `301`, preserve query string off.
...
```bash
for h in gate pool patio; do echo -n "$h -> "; curl -sI https://$h.example.com/ | awk 'tolower($1)=="location:"||$1=="HTTP/2"{print}'; done
```

Expected: each returns `301` with `location: https://home.example.com/`.
...
```bash
for h in gate pool patio; do echo -n "$h direct origin -> "; curl -sI https://$h.example.com/api/ | head -1; done
```
...
In `unifi-gate-instance/config/worker/wrangler.toml`, delete the `[[routes]]` blocks for `gate.example.com/*`, `gate-dev.example.com/*`, and `pool.example.com/*`. (The `home.example.com/*` route was already removed in a prior session.)
...
```bash
curl -sI https://gate.example.com/ | head -1   # still 301 to home (edge rule), Worker gone
```
...
curl -s -H 'X-Verified-User: home-gateway@local' http://10.0.0.240:8000/devices | head -c 80; echo
```
...
curl -sI https://home.example.com/ | head -1     # 200/302 via home tunnel
for h in gate pool patio; do echo -n "$h -> "; curl -sI https://$h.example.com/ | head -1; done  # all 301
```
...
- One public hostname (`home.example.com`); `gate.`/`pool.`/`patio.` 301 to it.
