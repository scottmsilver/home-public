...
def test_snapshot_one_control_per_door_plus_aggregate():
    ...
    # Closed/Open wording).
    assert c["legacy"].mode is None and c["legacy"].status == "Closed"
...
@responses.activate
def test_raw_attaches_canonical_derived_state():
    ...
    # genuinely unlocked.
    assert doors["garage"]["derived"]["open"] is True
    assert doors["garage"]["derived"]["label"] == "Open"
    # held forever -> open + forever mode.
    assert doors["side"]["derived"]["open"] is True and doors["side"]["derived"]["mode"] == "forever"
